...
export default function TeamsPage() {
  ...
  const tournamentId = getParamAsString(params?.id as string | string[] | undefined);
  ...
  const remainingCount = useMemo(
    () => teams.filter((t) => !(t.group_manual ?? false)).length,
    [teams]
  );
  ...
  const previewJersey = useMemo(
    () => genJerseySvg(jerseyStyle, selectedColors),
    [jerseyStyle, selectedColors]
  );
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      if (!tournamentId) {
        if (!alive) return;
        setStatus("Erreur: ID tournoi introuvable.");
        setLoading(false);
        return;
      }
      ...
      setTournament((tData ?? null) as TournamentRow);
  ...
  async function setTeamGroup(teamId: string, groupIdx: number) {
    ...
    const { error } = await supabase
      .from("teams")
      .update({ group_idx: idx, group_manual: true })
      .eq("id", teamId);
    ...
    setTeams((prev) =>
      prev.map((t) =>
        t.id === teamId ? { ...t, group_idx: idx, group_manual: true } : t
      )
    );
    setBusyTeam((p) => ({ ...p, [teamId]: false }));
  }
  ...
  async function autoAssignRemaining() {
    ...
    if (remaining.length === 0) {
      return setStatus("Aucune équipe restante à répartir.");
    }
      ...
      <div className="max-w-6xl mx-auto space-y-4">
        <div className="bg-white rounded-xl shadow p-6 flex items-start justify-between gap-3 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <button
              onClick={() => refreshTeams(tournamentId)}
              className="bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300 transition"
              title="Rafraîchir"
            >
              🔄
        ...
        <div className="bg-white rounded-xl shadow p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500 mt-2">
                  Sélection: {selectedColors.length}/3
                </div>
              </div>
              ...
              <div className="border rounded-lg p-3 bg-slate-50">
                ...
                <div className="flex gap-2 flex-wrap">
                  <button
                    type="button"
                    onClick={() =>
                      setLogoSvg(genLogoSvg(clean(name) || "Équipe", selectedColors))
                    }
              ...
              {showGroups && (
                <div className="text-xs text-gray-500">
                  Une équipe est “restante” tant que sa poule n’a pas été choisie manuellement.
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-50 border rounded-xl p-4 space-y-3">
              ...
              <div className="flex items-center gap-3">
                <div
                  className="w-24 h-24 rounded-xl overflow-hidden bg-white border flex items-center justify-center"
                  dangerouslySetInnerHTML={{ __html: previewJersey }}
                  title="Maillot"
                />

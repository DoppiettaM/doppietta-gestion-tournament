...
export default function ChallengeSettingsPage() {
  ...
  useEffect(() => { (async () => {
    ...
    const [{ data: c, error }, { data: e }] = await Promise.all([
      supabase.from("challenges").select("title,event_date,venue,field_names,match_duration_min,rotation_duration_min,display_theme,display_logo_url,display_banners,referee_names").eq("id", id).single(),

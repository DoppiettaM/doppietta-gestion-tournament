...
export default function ChallengeSettingsPage() {
  ...
  async function save() {
    if (!title.trim()) return setStatus("Le nom est obligatoire."); if (!venue.trim()) return setStatus("Le lieu du challenge est obligatoire : il apparaîtra sur les fiches d’effectif."); if (fields.some(f => !f.trim())) return setStatus("Chaque terrain doit avoir un nom.");

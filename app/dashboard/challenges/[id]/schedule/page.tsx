...
type SchedulingRules = { max_match_count_gap?: number; min_rest_slots?: number; prevent_simultaneous?: boolean; rest_policy?: "strict"; phase_transition_min?: number };
...
export default function SharedChallengeSchedulePage() {
  ...
  const [title, setTitle] = useState("Challenge"); const [shared, setShared] = useState(false); const [challengeFields, setChallengeFields] = useState<string[]>([]); const [referees, setReferees] = useState<string[]>([]); const [rules, setRules] = useState<SchedulingRules>({max_match_count_gap:1,min_rest_slots:1,prevent_simultaneous:true,rest_policy:"strict",phase_transition_min:10}); const [tournaments, setTournaments] = useState<Tournament[]>([]); const [teams, setTeams] = useState<Team[]>([]); const [matches, setMatches] = useState<Match[]>([]); const [status, setStatus] = useState("Chargement...");
  ...
  async function rebalance() {
      ...
      for(let round=0;round<Math.ceil(slots.length/fields)&&remaining.length;round++){
        ...
        for(let fieldOffset=0;fieldOffset<fields&&remaining.length;fieldOffset++){
          ...
          for(let i=0;i<remaining.length;i++){
            ...
            const consecutive=participants.some(id=>round-(lastRound.get(id)??-9999)<=minRest); if(consecutive)continue;
            ...
            const score=i; if(score<bestScore){best=i;bestScore=score;}
          }
          if(best<0)continue;
          const [match]=remaining.splice(best,1); const participants=[match.home_team_id,match.away_team_id].filter(Boolean) as string[];
          for(const id of participants){busy.add(id);counts.set(id,(counts.get(id)??0)+1);lastRound.set(id,round);} if(!String(match.stage??"").startsWith("phase_2"))lastPhaseOneRound.set(match.tournament_id,round); placed.push({match,slot:slotIndex});
        }
      }
      return {placed,remaining,counts};
    }
    const plan=buildPlan();
    if(plan.remaining.length)return setStatus(`Répartition refusée : ${plan.remaining.length} match(s) ne peuvent pas être placés en respectant le repos obligatoire d’un créneau et l’équité. Ajoutez des créneaux horaires ; aucun enchaînement ne sera autorisé.`);
    ...
    setStatus("Planning conforme : aucun match simultané, repos obligatoire des équipes et pause d’au moins 10 minutes entre les phases de chaque tournoi.");
  }
  ...
  return <main className="min-h-screen bg-slate-100 p-6"><div className="max-w-7xl mx-auto space-y-4"><header className="bg-slate-950 text-white rounded-2xl p-6 flex justify-between gap-3 flex-wrap"><div><p className="text-amber-400 font-bold text-sm">RESSOURCES PARTAGÉES</p><h1 className="text-3xl font-black">Planning global · {title}</h1><p className="text-white/60">{matches.length} matchs · {conflicts.size} collision(s) de terrain · {pauseConflicts.size} match(s) sur une pause</p><p className="mt-1 text-amber-300">{consecutiveMatches.length} enchaînement(s) de deux matchs consécutifs à surveiller</p></div><div className="flex gap-2"><button onClick={rebalance} className="bg-amber-400 text-slate-950 px-4 py-2 rounded-xl font-bold">Répartir sans conflit</button><button onClick={() => router.push(`/dashboard/challenges/${challengeId}`)} className="bg-white/10 px-4 py-2 rounded-xl">← Challenge</button></div></header>
    ...
    <section className={`rounded-2xl border p-5 shadow ${consecutiveMatches.length?"border-red-300 bg-red-50":"border-emerald-200 bg-emerald-50"}`}><div className="flex items-center justify-between gap-3"><div><h2 className="font-black">Contrôle du repos obligatoire</h2><p className="text-sm text-gray-600">Toute équipe doit laisser passer au moins un créneau complet avant de rejouer. Une violation bloque la conformité du planning.</p></div><strong className={`rounded-full px-3 py-1 ${consecutiveMatches.length?"bg-red-200 text-red-900":"bg-emerald-200 text-emerald-900"}`}>{consecutiveMatches.length}</strong></div>{consecutiveMatches.length?<div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{consecutiveMatches.map(row=><div key={row.key} className="rounded-xl border border-red-200 bg-white p-3 text-sm"><strong className="block break-words">{row.mark} {row.team}</strong><span className="text-red-800">{row.from} → {row.to} · violation à corriger</span></div>)}</div>:<p className="mt-3 text-sm font-semibold text-emerald-800">Aucun enchaînement : le planning respecte la récupération.</p>}</section>
    <section className="grid gap-3 md:grid-cols-4"><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><strong>Équité obligatoire</strong><span className="block text-sm text-gray-600">Écart maximal enregistré : 1 match.</span></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><strong>Double présence interdite</strong><span className="block text-sm text-gray-600">Contrôle sur tous les terrains du challenge.</span></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><strong>Récupération obligatoire</strong><span className="block text-sm text-gray-600">Au moins {Math.max(1,Number(rules.min_rest_slots??1))} créneau(x) entièrement libre entre deux matchs, sans exception.</span></div><div className="rounded-2xl border border-violet-200 bg-violet-50 p-4"><strong>Transition de phase</strong><span className="block text-sm text-gray-600">Pause minimale de 10 minutes par tournoi ; les autres tournois peuvent continuer.</span></div></section>

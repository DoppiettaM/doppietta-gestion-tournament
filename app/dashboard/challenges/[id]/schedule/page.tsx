...
type Match = { id: string; tournament_id: string; start_time: string; field_idx: number; stage: string | null; home_team_id: string | null; away_team_id: string | null; home_source_label: string | null; away_source_label: string | null; home: { name: string } | null; away: { name: string } | null; referee_label: string | null };
type Team = { id: string; name: string; tournament_id: string };
type SchedulingRules = { max_match_count_gap?: number; min_rest_slots?: number; prevent_simultaneous?: boolean; rest_policy?: "one_exception_per_day"; max_consecutive_exceptions?: number; phase_transition_min?: number };
...
export default function SharedChallengeSchedulePage() {
  ...
  async function refresh() {
    ...
    const [{ data: ts, error: te }, { data: teamRows, error: teamError }, { data: ms, error: me }] = await Promise.all([
      ...
      supabase.from("matches").select("id,tournament_id,start_time,field_idx,stage,home_team_id,away_team_id,home_source_label,away_source_label,referee_label,home:home_team_id(name),away:away_team_id(name)").in("tournament_id", ids).order("start_time").order("field_idx")
    ]); if (te || teamError || me) return setStatus("Erreur: " + (te?.message ?? teamError?.message ?? me?.message));
    setTitle(c.title); setShared(c.shared_resources); setChallengeFields(Array.isArray(c.field_names)?c.field_names:[]); setReferees(Array.isArray(c.referee_names)&&c.referee_names.length?c.referee_names:Array.from({length:12},(_,i)=>`Arbitre ${i+1}`)); setRules({...((c.scheduling_rules&&typeof c.scheduling_rules==="object"?c.scheduling_rules:{}) as SchedulingRules),max_match_count_gap:1,min_rest_slots:Math.max(1,Number(c.scheduling_rules?.min_rest_slots??1)),prevent_simultaneous:true,rest_policy:"one_exception_per_day",max_consecutive_exceptions:1,phase_transition_min:Math.max(10,Number(c.scheduling_rules?.phase_transition_min??10))}); setTournaments((ts ?? []) as Tournament[]); setTeams((teamRows??[]) as Team[]); setMatches((ms ?? []) as unknown as Match[]); setStatus("");
  }
  ...
  async function rebalance() {
    ...
    function buildPlan() {
      const remaining=[...ordered]; const counts=new Map(teamIds.map(id=>[id,0])); const lastRound=new Map<string,number>(); const consecutiveUses=new Map(teamIds.map(id=>[id,0])); const placed:Array<{match:Match;slot:number}>=[];
      const lastPhaseOneRound=new Map<string,number>();
      for(let round=0;round<Math.ceil(slots.length/fields)&&remaining.length;round++){
        ...
        for(let fieldOffset=0;fieldOffset<fields&&remaining.length;fieldOffset++){
          ...
          for(let i=0;i<remaining.length;i++){
            ...
            const phaseTwo=String(match.stage??"").startsWith("phase_2");
            const phaseOneRemaining=remaining.some(row=>row.tournament_id===match.tournament_id&&!String(row.stage??"").startsWith("phase_2"));
            const transitionRounds=Math.max(1,Math.ceil((Number(tournament?.match_duration_min??slot)+Math.max(10,Number(rules.phase_transition_min??10)))/slot));
            if(phaseTwo&&(phaseOneRemaining||round-(lastPhaseOneRound.get(match.tournament_id)??-9999)<transitionRounds))continue;
            ...
            const consecutiveTeams=participants.filter(id=>round-(lastRound.get(id)??-9999)<=minRest);
            if(consecutiveTeams.some(id=>(consecutiveUses.get(id)??0)>=1))continue;
            ...
            const score=i+consecutiveTeams.length*10000; if(score<bestScore){best=i;bestScore=score;}
          }
          if(best<0)continue;
          const [match]=remaining.splice(best,1); const participants=[match.home_team_id,match.away_team_id].filter(Boolean) as string[];
          for(const id of participants){if(round-(lastRound.get(id)??-9999)<=minRest)consecutiveUses.set(id,(consecutiveUses.get(id)??0)+1);busy.add(id);counts.set(id,(counts.get(id)??0)+1);lastRound.set(id,round);} if(!String(match.stage??"").startsWith("phase_2"))lastPhaseOneRound.set(match.tournament_id,round); placed.push({match,slot:slotIndex});
        }
      }
      return {placed,remaining,counts,consecutiveUses};
    }
    const plan=buildPlan();
    if(plan.remaining.length)return setStatus(`Répartition refusée : ${plan.remaining.length} match(s) ne peuvent pas être placés sans dépasser la limite d’un seul enchaînement par équipe sur la journée.`);
    ...
    const results = await Promise.all(placed.map(({ match, slot: index }, order) => supabase.from("matches").update({ ...slots[index], referee_team_id: null, referee_label: names[(Math.floor(index/fields)*fields+(index%fields))%names.length], schedule_order: order + 1 }).eq("id", match.id))); const error = results.find(x => x.error)?.error; if (error) return setStatus("Erreur: " + error.message); await refresh();
    const exceptions=Array.from(plan.consecutiveUses.values()).filter(value=>value>0).length;
    setStatus(`Planning optimisé : ${exceptions} équipe(s) utilisent leur unique exception d’enchaînement ; aucune ne la dépasse. Pause de 10 minutes entre les phases respectée.`);
  }
  ...
  return <main className="min-h-screen bg-slate-100 p-6"><div className="max-w-7xl mx-auto space-y-4"><header className="bg-slate-950 text-white rounded-2xl p-6 flex justify-between gap-3 flex-wrap"><div><p className="text-amber-400 font-bold text-sm">RESSOURCES PARTAGÉES</p><h1 className="text-3xl font-black">Planning global · {title}</h1><p className="text-white/60">{matches.length} matchs · {conflicts.size} collision(s) de terrain · {pauseConflicts.size} match(s) sur une pause</p><p className="mt-1 text-amber-300">{consecutiveMatches.length} enchaînement(s) de deux matchs consécutifs à surveiller</p></div><div className="flex gap-2"><button onClick={rebalance} className="bg-amber-400 text-slate-950 px-4 py-2 rounded-xl font-bold">Répartir sans conflit</button><button onClick={() => router.push(`/dashboard/challenges/${challengeId}`)} className="bg-white/10 px-4 py-2 rounded-xl">← Challenge</button></div></header>
    ...
    <section className={`rounded-2xl border p-5 shadow ${consecutiveMatches.length?"border-amber-300 bg-amber-50":"border-emerald-200 bg-emerald-50"}`}><div className="flex items-center justify-between gap-3"><div><h2 className="font-black">Repos et exceptions d’enchaînement</h2><p className="text-sm text-gray-600">Le repos d’un créneau est fortement conseillé. Une équipe peut exceptionnellement enchaîner une seule fois sur toute la journée.</p></div><strong className={`rounded-full px-3 py-1 ${consecutiveMatches.length?"bg-amber-200 text-amber-900":"bg-emerald-200 text-emerald-900"}`}>{consecutiveMatches.length}</strong></div>{consecutiveMatches.length?<div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{consecutiveMatches.map(row=><div key={row.key} className="rounded-xl border border-amber-200 bg-white p-3 text-sm"><strong className="block break-words">{row.mark} {row.team}</strong><span className="text-amber-800">{row.from} → {row.to} · exception utilisée</span></div>)}</div>:<p className="mt-3 text-sm font-semibold text-emerald-800">Aucun enchaînement nécessaire dans ce planning.</p>}</section>
    <section className="grid gap-3 md:grid-cols-4"><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><strong>Équité obligatoire</strong><span className="block text-sm text-gray-600">Écart maximal enregistré : 1 match.</span></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><strong>Double présence interdite</strong><span className="block text-sm text-gray-600">Contrôle sur tous les terrains du challenge.</span></div><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><strong>Récupération optimisée</strong><span className="block text-sm text-gray-600">Au moins {Math.max(1,Number(rules.min_rest_slots??1))} créneau(x) libre conseillé ; une seule exception maximum par équipe et par jour.</span></div><div className="rounded-2xl border border-violet-200 bg-violet-50 p-4"><strong>Transition de phase</strong><span className="block text-sm text-gray-600">Pause minimale de 10 minutes par tournoi ; les autres tournois peuvent continuer.</span></div></section>

...
type Tournament = { id: string; title: string; display_label: string | null; start_time: string; end_time: string; match_duration_min: number; rotation_duration_min: number; num_fields: number; field_names: string[] };
...
export default function SharedChallengeSchedulePage() {
  ...
  async function refresh() {
    ...
    const [{ data: ts, error: te }, { data: ms, error: me }] = await Promise.all([
      supabase.from("tournaments").select("id,title,display_label,start_time,end_time,match_duration_min,rotation_duration_min,num_fields,field_names").in("id", ids),
  ...
  async function rebalance() {
    ...
    const results = await Promise.all(placed.map(({ match, slot: index }, order) => supabase.from("matches").update({ ...slots[index], referee_team_id: null, referee_label: `Arbitre ${(index % 12) + 1}`, schedule_order: order + 1 }).eq("id", match.id))); const error = results.find(x => x.error)?.error; if (error) return setStatus("Erreur: " + error.message); await refresh(); setStatus("Planning commun réorganisé sans collision de terrain ni d’arbitre.");
  }
  ...
  return <main className="min-h-screen bg-slate-100 p-6"><div className="max-w-7xl mx-auto space-y-4"><header className="bg-slate-950 text-white rounded-2xl p-6 flex justify-between gap-3 flex-wrap"><div><p className="text-amber-400 font-bold text-sm">RESSOURCES PARTAGÉES</p><h1 className="text-3xl font-black">Planning global · {title}</h1><p className="text-white/60">{matches.length} matchs · {conflicts.size} collision(s)</p></div><div className="flex gap-2"><button onClick={rebalance} className="bg-amber-400 text-slate-950 px-4 py-2 rounded-xl font-bold">Répartir sans conflit</button><button onClick={() => router.push(`/dashboard/challenges/${challengeId}`)} className="bg-white/10 px-4 py-2 rounded-xl">← Challenge</button></div></header>
    ...
    <section className="grid lg:grid-cols-4 gap-4"><div className="lg:col-span-3 bg-white rounded-2xl shadow p-6 space-y-2">{matches.map(match => { const key = `${hhmm(match.start_time)}|${match.field_idx}`; const tournament = tournamentMap.get(match.tournament_id); const fieldName = tournament?.field_names?.[match.field_idx - 1] ?? `Terrain ${match.field_idx}`; return <div key={match.id} className={`border rounded-xl p-3 grid md:grid-cols-4 gap-2 ${conflicts.has(key) ? "border-red-400 bg-red-50" : ""}`}><strong>{hhmm(match.start_time)} · {fieldName}</strong><span className="text-xs font-bold text-gray-500">{tournament?.display_label ?? tournament?.title}</span><span>{match.home?.name ?? match.home_source_label ?? "À déterminer"} — {match.away?.name ?? match.away_source_label ?? "À déterminer"}</span><span className="text-sm text-gray-500">Arbitre: {match.referee_label ?? "à attribuer"}</span></div>; })}</div><aside className="bg-white rounded-2xl shadow p-5"><h2 className="font-black">Équité des matchs</h2>{teamCounts.map(team => <div key={team.name} className="flex justify-between border-b py-2 text-sm"><span>{team.name}</span><strong>{team.count}</strong></div>)}</aside></section>
  </div></main>;
}

...
type Challenge = { id: string; title: string; scoring_mode: ScoringMode; default_points_by_rank: number[]; tie_breakers: TieBreaker[]; shared_resources: boolean; field_names:string[]; referee_names:string[] };
type Link = { tournament_id: string; position: number; points_by_rank: number[] | null; tournament: { id: string; title: string; display_label?:string|null; format?:string|null; tournament_date: string | null; scoring_rules?: { win?: number; draw?: number; loss?: number; goal_bonus?: number } | null } | null };
...
export default function ChallengeDashboardPage() {
  ...
  async function refresh() {
    const { data: c, error } = await supabase.from("challenges").select("id,title,scoring_mode,default_points_by_rank,tie_breakers,shared_resources,field_names,referee_names").eq("id", challengeId).single();
    if (error) return setStatus("Erreur challenge: " + error.message);
    const { data: l, error: le } = await supabase.from("challenge_tournaments").select("tournament_id,position,points_by_rank,tournament:tournament_id(id,title,display_label,format,tournament_date,scoring_rules)").eq("challenge_id", challengeId).order("position");

...
type Link = { tournament_id: string; position: number; points_by_rank: number[] | null; tournament: { id: string; title: string; tournament_date: string | null; scoring_rules?: { win?: number; draw?: number; loss?: number; goal_bonus?: number } | null } | null };
...
export default function ChallengeDashboardPage() {
  ...
  async function refresh() {
    ...
    const { data: l, error: le } = await supabase.from("challenge_tournaments").select("tournament_id,position,points_by_rank,tournament:tournament_id(id,title,tournament_date,scoring_rules)").eq("challenge_id", challengeId).order("position");

    ...
    >
      ...
      <ul
        className={[
          ...
          highlight ? "text-slate-700" : "text-white/85",
        ].join(" ")}
      >
        {features.map((f) => (
...
export default function HomePage() {
  ...
  async function handleSignupTrial() {
    ...
    if (error) return setStatus("❌ Création impossible: " + error.message);
  ...
  async function handleRequestPlan(plan: Plan) {
    ...
    try {
      ...
      if (error) {
        return setStatus(
          "⚠️ Demande non envoyée (table account_requests absente ou droits). Si tu veux, je te donne le SQL."
        );
      }
    } catch {
      setBusy(false);
      return setStatus("⚠️ Demande non envoyée. Si tu veux, je te donne le SQL.");
    }
    ...
    <main className="min-h-screen bg-slate-950 text-white">
          ...
          <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            <div>
              ...
              <p className="mt-4 text-white/80 text-base sm:text-lg">
                Planifie, gère les matchs, affiche l’écran géant, calcule les classements et centralise les résultats.
              </p>
              ...
              <div className="mt-8 grid grid-cols-3 gap-3">
                {["/gallery/event-1.jpg", "/gallery/event-2.jpg", "/gallery/event-3.jpg"].map((src) => (
              ...
              <p className="mt-2 text-xs text-white/50">
                Mets tes photos dans <span className="font-mono">public/gallery/</span> :
                <span className="font-mono"> event-1.jpg</span>, <span className="font-mono">event-2.jpg</span>,{" "}
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-sm">
              ...
              <div className="mt-5 space-y-3">
                ...
                <div className="mt-3 text-xs text-white/55">
                  Si Supabase demande confirmation email, valide l’email avant de te connecter.
                </div>
              </div>

              <div className="mt-6 rounded-3xl border border-white/10 bg-black/20 p-5">
                <div className="text-sm font-extrabold">Passer à Starter / Premium</div>
                <div className="mt-1 text-xs text-white/60">Demande “en attente”, validation par admin.</div>
                ...
                <div className="mt-2 text-[11px] text-white/55">
                  Table Supabase recommandée: <span className="font-mono">account_requests</span>.
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-6 py-12">
          <div className="flex items-end justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="mt-1 text-sm text-white/70">Placeholders, tu remplaces par logos réels ensuite.</p>
            </div>
          </div>
      ...
      <section className="bg-gradient-to-b from-slate-950 to-slate-900">
        <div className="mx-auto max-w-7xl px-6 py-12">
          ...
          <p className="mt-1 text-sm text-white/70">
            Essai gratuit, puis déblocage Starter/Premium.
          </p>
          ...
          <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
            <PriceCard
              title="Essai"
              price="0€"
              subtitle="Tester un événement"
              features={["Création tournoi", "Planning / Matchs / Écran", "Résultats + classements", "PDF basique"]}
            ...
            <div className="rounded-3xl border border-blue-200 bg-white p-1">
              <div className="rounded-3xl bg-white p-5">
                <PriceCard
                  title="Starter"
                  price="€"
                  subtitle="Club / petit tournoi"
                  features={["Terrains & poules", "Auto-refresh", "Exports PDF", "Support prioritaire"]}
                  highlight
                  cta="Demander Starter"
                  onClick={() => handleRequestPlan("starter")}
                />
              </div>
            </div>
            ...
            <PriceCard
              title="Premium"
              price="€€"
              subtitle="Événements + sponsors"
              features={["Écran avancé", "Zones sponsors", "Stats enrichies", "Accompagnement"]}
      ...
      <footer className="bg-slate-950 border-t border-white/10">
        <div className="mx-auto max-w-7xl px-6 py-8 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          ...
          <div className="text-xs text-white/50">© {new Date().getFullYear()} Doppietta Gestion Tournament.</div>
        </div>
      </footer>
    </main>
  );
